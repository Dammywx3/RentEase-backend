...
\restrict MAxFJe1S0Gbj2hcQHjUNG2vF4MKbekk5ck8a4jgGRhNca0CuMVfUydmcfDPGfEK
...
--
-- Name: escrow_txn_type; Type: TYPE; Schema: public; Owner: -
--

CREATE TYPE public.escrow_txn_type AS ENUM (
    'deposit_in',
    'deposit_out',
    'fee_in',
    'fee_out',
...
--
-- Name: purchase_audit_event_type; Type: TYPE; Schema: public; Owner: -
--

CREATE TYPE public.purchase_audit_event_type AS ENUM (
    'created',
    'status_changed',
    'paid',
    'completed',
...
--
-- Name: purchase_event_type; Type: TYPE; Schema: public; Owner: -
--
...
--
-- Name: purchase_payment_status; Type: TYPE; Schema: public; Owner: -
--
...
--
-- Name: wallet_account_purpose; Type: TYPE; Schema: public; Owner: -
--

CREATE TYPE public.wallet_account_purpose AS ENUM (
    'user',
    'platform',
    'escrow'
);
...
--
-- Name: assert_purchase_transition(text, text, text, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.assert_purchase_transition(old_status text, new_status text, old_pay text, new_pay text) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
  old_rank int;
...
--
-- Name: current_app_event_note(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.current_app_event_note() RETURNS text
    LANGUAGE sql STABLE
    AS $$
...
--
-- Name: current_app_org_uuid(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.current_app_org_uuid() RETURNS uuid
    LANGUAGE sql STABLE
    AS $$
...
--
-- Name: current_app_user_uuid(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.current_app_user_uuid() RETURNS uuid
    LANGUAGE sql STABLE
    AS $$
...
--
-- Name: get_or_create_org_escrow_wallet(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.get_or_create_org_escrow_wallet(p_org uuid, p_currency text) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
  v_wallet_id uuid;
BEGIN
  ...
  INSERT INTO public.wallet_accounts (id, organization_id, user_id, currency, is_platform_wallet, created_at, updated_at)
  VALUES (rentease_uuid(), p_org, NULL, p_currency, TRUE, now(), now())
...
--
-- Name: get_or_create_org_platform_wallet(uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.get_or_create_org_platform_wallet(p_org uuid, p_currency text) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
  v_wallet_id uuid;
  ...
  v_ccy text;
BEGIN
  v_ccy := upper(trim(coalesce(p_currency,'')));
  IF v_ccy = '' THEN
  ...
  IF v_has_purpose THEN
    SELECT wa.id INTO v_wallet_id
    FROM public.wallet_accounts wa
    WHERE wa.organization_id = p_org
      AND wa.user_id IS NULL
      AND wa.currency = v_ccy
      AND wa.purpose = 'platform'
    LIMIT 1;
    ...
    VALUES (rentease_uuid(), p_org, NULL, v_ccy, TRUE, 'platform', now(), now())
    RETURNING id INTO v_wallet_id;

    RETURN v_wallet_id;
  ELSE
    -- fallback: best-effort with existing columns only
    SELECT wa.id INTO v_wallet_id
    FROM public.wallet_accounts wa
    WHERE wa.organization_id = p_org
      AND wa.user_id IS NULL
    ...
    IF v_wallet_id IS NULL THEN
      INSERT INTO public.wallet_accounts (id, organization_id, user_id, currency, is_platform_wallet, created_at, updated_at)
      VALUES (rentease_uuid(), p_org, NULL, v_ccy, TRUE, now(), now())
      RETURNING id INTO v_wallet_id;
    END IF;
...
--
-- Name: get_or_create_user_wallet_account(uuid, uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.get_or_create_user_wallet_account(p_org uuid, p_user uuid, p_currency text) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
declare
  v_wallet_id uuid;
begin
  ...
  insert into public.wallet_accounts (id, organization_id, user_id, currency, is_platform_wallet, created_at, updated_at)
  values (rentease_uuid(), p_org, p_user, p_currency, false, now(), now())
  returning id into v_wallet_id;
...
--
-- Name: get_user_wallet_account(uuid, uuid, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.get_user_wallet_account(p_org uuid, p_user uuid, p_currency text) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
  v_wallet_id uuid;
...
BEGIN
  ...
  IF v_has_purpose THEN
    ...
    INSERT INTO public.wallet_accounts (id, organization_id, user_id, currency, is_platform_wallet, purpose, created_at, updated_at)
    VALUES (rentease_uuid(), p_org, p_user, v_ccy, FALSE, 'user', now(), now())
    RETURNING id INTO v_wallet_id;

    RETURN v_wallet_id;
  ELSE
    -- older schema fallback (no purpose column)
    SELECT wa.id INTO v_wallet_id
    FROM public.wallet_accounts wa
    WHERE wa.organization_id = p_org
      AND wa.user_id = p_user
    ...
    INSERT INTO public.wallet_accounts (id, organization_id, user_id, currency, is_platform_wallet, created_at, updated_at)
    VALUES (rentease_uuid(), p_org, p_user, v_ccy, FALSE, now(), now())
    RETURNING id INTO v_wallet_id;
...
--
-- Name: purchase_escrow_hold(uuid, uuid, numeric, text, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.purchase_escrow_hold(p_purchase_id uuid, p_org uuid, p_new_total_amount numeric, p_currency text, p_note text DEFAULT NULL::text) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
  v_purchase record;
...
BEGIN
  IF p_new_total_amount IS NULL OR p_new_total_amount <= 0 THEN
    RAISE EXCEPTION 'Escrow hold amount must be > 0';
  END IF;
  ...
  -- Currency must match purchase currency
  IF v_purchase.currency IS DISTINCT FROM p_currency THEN
    RAISE EXCEPTION 'Currency mismatch: purchase=% but request=%', v_purchase.currency, p_currency;
  END IF;

  -- If already fully released, don't allow holding again (prevents weird states)
  IF coalesce(v_purchase.escrow_released_amount,0) >= coalesce(v_purchase.escrow_held_amount,0)
     AND coalesce(v_purchase.escrow_released_amount,0) > 0 THEN
    RAISE EXCEPTION 'Cannot hold escrow after escrow has been released';
  END IF;

  v_delta := p_new_total_amount - coalesce(v_purchase.escrow_held_amount, 0);
  IF v_delta <= 0 THEN
    RETURN;
  END IF;

  v_escrow_wallet := public.get_or_create_org_escrow_wallet(p_org, p_currency);
  v_buyer_wallet := public.get_user_wallet_account(p_org, v_purchase.buyer_id, p_currency);

  INSERT INTO public.wallet_transactions (
    id, organization_id, wallet_account_id, txn_type,
    reference_type, reference_id, amount, currency, note
  )
  VALUES (
    rentease_uuid(), p_org, v_buyer_wallet, 'debit_buyer'::wallet_transaction_type,
    'purchase', p_purchase_id, v_delta, p_currency, coalesce(p_note, 'escrow hold')
  )
  ON CONFLICT (wallet_account_id, reference_type, reference_id, txn_type)
  WHERE (reference_type='purchase')
  DO UPDATE SET amount = public.wallet_transactions.amount + EXCLUDED.amount,
                note = EXCLUDED.note;
  ...
  VALUES (
    rentease_uuid(), p_org, v_escrow_wallet, 'credit_escrow'::wallet_transaction_type,
    'purchase', p_purchase_id, v_delta, p_currency, coalesce(p_note, 'escrow hold')
  )
  ON CONFLICT (wallet_account_id, reference_type, reference_id, txn_type)
  WHERE (reference_type='purchase')
  ...
  SET escrow_wallet_account_id = v_escrow_wallet,
      escrow_held_amount = p_new_total_amount,
      updated_at = now()
  WHERE id = p_purchase_id
    AND organization_id = p_org;
END;
...
--
-- Name: purchase_escrow_release(uuid, uuid, numeric, text, numeric, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.purchase_escrow_release(p_purchase_id uuid, p_org uuid, p_new_total_release numeric, p_currency text, p_platform_fee numeric DEFAULT 0, p_note text DEFAULT NULL::text) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
  v_purchase record;
  ...
  v_delta numeric;
  v_seller_delta numeric;
BEGIN
  IF p_new_total_release IS NULL OR p_new_total_release <= 0 THEN
    RAISE EXCEPTION 'Escrow release amount must be > 0';
  END IF;
  IF p_platform_fee IS NULL OR p_platform_fee < 0 THEN
    RAISE EXCEPTION 'Platform fee must be >= 0';
  END IF;
  ...
  IF p_new_total_release > coalesce(v_purchase.escrow_held_amount,0) THEN
    RAISE EXCEPTION 'Cannot release % because held is %', p_new_total_release, coalesce(v_purchase.escrow_held_amount,0);
  END IF;

  v_escrow_wallet := v_purchase.escrow_wallet_account_id;
  IF v_escrow_wallet IS NULL THEN
    RAISE EXCEPTION 'Escrow wallet not set on purchase';
  END IF;

  v_delta := p_new_total_release - coalesce(v_purchase.escrow_released_amount, 0);
  IF v_delta <= 0 THEN
    RETURN;
  END IF;

  IF p_platform_fee > v_delta THEN
    RAISE EXCEPTION 'Platform fee % cannot exceed release delta %', p_platform_fee, v_delta;
  END IF;

  v_seller_delta := v_delta - p_platform_fee;

  v_seller_wallet := public.get_user_wallet_account(p_org, v_purchase.seller_id, p_currency);

  SELECT wa.id INTO v_platform_wallet
  FROM public.wallet_accounts wa
  WHERE wa.organization_id = p_org
    AND wa.user_id IS NULL
    AND wa.currency = p_currency
  ...
  IF v_platform_wallet IS NULL THEN
    RAISE EXCEPTION 'Platform wallet not found for org=% currency=%', p_org, p_currency;
  END IF;
  ...
  VALUES (
    rentease_uuid(), p_org, v_escrow_wallet, 'debit_escrow'::wallet_transaction_type,
    'purchase', p_purchase_id, v_delta, p_currency, coalesce(p_note, 'escrow release')
  )
  ON CONFLICT (wallet_account_id, reference_type, reference_id, txn_type)
  WHERE (reference_type='purchase')
  DO UPDATE SET amount = public.wallet_transactions.amount + EXCLUDED.amount,
  ...
  IF v_seller_delta > 0 THEN
    INSERT INTO public.wallet_transactions (
      id, organization_id, wallet_account_id, txn_type,
      reference_type, reference_id, amount, currency, note
    )
    VALUES (
      rentease_uuid(), p_org, v_seller_wallet, 'credit_seller'::wallet_transaction_type,
      'purchase', p_purchase_id, v_seller_delta, p_currency, coalesce(p_note, 'escrow release')
    )
    ON CONFLICT (wallet_account_id, reference_type, reference_id, txn_type)
    WHERE (reference_type='purchase')
    DO UPDATE SET amount = public.wallet_transactions.amount + EXCLUDED.amount,
  ...
  IF p_platform_fee > 0 THEN
    ...
    VALUES (
      rentease_uuid(), p_org, v_platform_wallet, 'credit_platform_fee'::wallet_transaction_type,
      'purchase', p_purchase_id, p_platform_fee, p_currency, coalesce(p_note, 'platform fee')
    )
    ON CONFLICT (wallet_account_id, reference_type, reference_id, txn_type)
    WHERE (reference_type='purchase')
    DO UPDATE SET amount = public.wallet_transactions.amount + EXCLUDED.amount,
  ...
  UPDATE public.property_purchases
  SET escrow_released_amount = p_new_total_release,
...
--
-- Name: purchase_status_rank(text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.purchase_status_rank(s text) RETURNS integer
...
--
-- Name: trg_property_purchases_transition_and_log(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.trg_property_purchases_transition_and_log() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
  old_status text;
...
CREATE TABLE public.escrow_accounts (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT escrow_accounts_balance_check CHECK ((balance >= (0)::numeric))
);
...
CREATE TABLE public.escrow_transactions (
    ...
    purchase_id uuid NOT NULL,
    txn_type public.escrow_txn_type NOT NULL,
...
--
-- Name: escrow_wallet_accounts; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.escrow_wallet_accounts (
...
--
-- Name: property_purchase_events; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.property_purchase_events (
    id uuid DEFAULT public.rentease_uuid() NOT NULL,
    organization_id uuid DEFAULT public.current_organization_uuid() NOT NULL,
    purchase_id uuid NOT NULL,
...
--
-- Name: property_purchase_payments; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.property_purchase_payments (
    id uuid DEFAULT public.rentease_uuid() NOT NULL,
    organization_id uuid DEFAULT public.current_organization_uuid() NOT NULL,
    purchase_id uuid NOT NULL,
    payment_id uuid NOT NULL,
    amount numeric(15,2) NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT property_purchase_payments_amount_check CHECK ((amount > (0)::numeric))
);


--
-- Name: property_purchases; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.property_purchases (
    ...
    currency character varying(3) DEFAULT 'USD'::character varying NOT NULL,
    earnest_deposit_amount numeric(15,2),
    closing_costs_estimate numeric(15,2),
    offer_expires_at timestamp with time zone,
    contract_signed_at timestamp with time zone,
    escrow_opened_at timestamp with time zone,
    closing_scheduled_at timestamp with time zone,
    closed_at timestamp with time zone,
    cancelled_at timestamp with time zone,
    status public.purchase_status DEFAULT 'initiated'::public.purchase_status NOT NULL,
    ...
    deleted_at timestamp with time zone,
    version integer DEFAULT 1,
    payment_status public.purchase_payment_status DEFAULT 'unpaid'::public.purchase_payment_status NOT NULL,
    ...
    escrow_wallet_account_id uuid,
    escrow_held_amount numeric(12,2) DEFAULT 0 NOT NULL,
    escrow_released_amount numeric(12,2) DEFAULT 0 NOT NULL,
    escrow_released_at timestamp with time zone,
    deposit_amount numeric(12,2),
    CONSTRAINT chk_purchase_paid_amount_nonneg CHECK (((paid_amount IS NULL) OR (paid_amount >= (0)::numeric))),
    CONSTRAINT chk_purchase_paid_amount_not_more_than_agreed CHECK (((paid_amount IS NULL) OR (paid_amount <= agreed_price))),
    CONSTRAINT chk_purchase_refunded_amount_nonneg CHECK (((refunded_amount IS NULL) OR (refunded_amount >= (0)::numeric))),
    CONSTRAINT property_purchases_agreed_price_check CHECK ((agreed_price > (0)::numeric)),
    CONSTRAINT property_purchases_closing_costs_estimate_check CHECK (((closing_costs_estimate IS NULL) OR (closing_costs_estimate >= (0)::numeric))),
    CONSTRAINT property_purchases_earnest_deposit_amount_check CHECK (((earnest_deposit_amount IS NULL) OR (earnest_deposit_amount >= (0)::numeric)))
);
...
CREATE TABLE public.purchase_events (
    ...
    organization_id uuid NOT NULL,
    purchase_id uuid NOT NULL,
    event_type public.purchase_audit_event_type NOT NULL,
    actor_user_id uuid,
    from_status text,
    to_status text,
    note text,
...
--
-- Name: rent_invoice_payments; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.users (
    ...
    deleted_at timestamp with time zone,
    version integer DEFAULT 1,
    account_locked_permanent boolean DEFAULT false NOT NULL,
    lock_stage integer DEFAULT 0 NOT NULL,
...
CREATE TABLE public.wallet_accounts (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    purpose public.wallet_account_purpose DEFAULT 'user'::public.wallet_account_purpose NOT NULL
);
...
--
-- Name: escrow_wallet_accounts escrow_wallet_accounts_organization_id_currency_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.escrow_wallet_accounts
    ADD CONSTRAINT escrow_wallet_accounts_organization_id_currency_key UNIQUE (organization_id, currency);


--
-- Name: escrow_wallet_accounts escrow_wallet_accounts_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.escrow_wallet_accounts
    ADD CONSTRAINT escrow_wallet_accounts_pkey PRIMARY KEY (id);
...
--
-- Name: property_purchase_events property_purchase_events_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchase_events
    ADD CONSTRAINT property_purchase_events_pkey PRIMARY KEY (id);


--
-- Name: property_purchase_payments property_purchase_payments_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchase_payments
    ADD CONSTRAINT property_purchase_payments_pkey PRIMARY KEY (id);


--
-- Name: property_purchases property_purchases_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchases
    ADD CONSTRAINT property_purchases_pkey PRIMARY KEY (id);
...
--
-- Name: rent_invoice_payments rent_invoice_payments_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: rental_applications uniq_application_per_listing_user; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.rental_applications
    ADD CONSTRAINT uniq_application_per_listing_user UNIQUE (listing_id, applicant_id);
...
--
-- Name: property_purchase_payments uniq_purchase_payment; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchase_payments
    ADD CONSTRAINT uniq_purchase_payment UNIQUE (purchase_id, payment_id);
...
--
-- Name: idx_escrow_accounts_org; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_escrow_accounts_org ON public.escrow_accounts USING btree (organization_id, created_at DESC);


--
-- Name: idx_escrow_txn_escrow; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_escrow_txn_escrow ON public.escrow_transactions USING btree (escrow_account_id, created_at DESC);


--
-- Name: idx_escrow_txn_purchase; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_escrow_txn_purchase ON public.escrow_transactions USING btree (purchase_id, created_at DESC);
...
--
-- Name: idx_property_purchases_buyer; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_property_purchases_buyer ON public.property_purchases USING btree (buyer_id, created_at DESC);


--
-- Name: idx_property_purchases_org; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_property_purchases_org ON public.property_purchases USING btree (organization_id, created_at DESC);


--
-- Name: idx_property_purchases_payment_status; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_property_purchases_payment_status ON public.property_purchases USING btree (payment_status, created_at DESC);


--
-- Name: idx_property_purchases_property; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_property_purchases_property ON public.property_purchases USING btree (property_id, created_at DESC);


--
-- Name: idx_property_purchases_refund_method; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_property_purchases_refund_method ON public.property_purchases USING btree (refund_method, refunded_at DESC) WHERE (refund_method IS NOT NULL);


--
-- Name: idx_property_purchases_refund_payment_id; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_property_purchases_refund_payment_id ON public.property_purchases USING btree (refund_payment_id) WHERE (refund_payment_id IS NOT NULL);


--
-- Name: idx_property_purchases_refunded_at; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_property_purchases_refunded_at ON public.property_purchases USING btree (refunded_at DESC) WHERE (refunded_at IS NOT NULL);


--
-- Name: idx_property_purchases_refunded_by; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_property_purchases_refunded_by ON public.property_purchases USING btree (refunded_by, refunded_at DESC) WHERE (refunded_by IS NOT NULL);


--
-- Name: idx_property_purchases_seller; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_property_purchases_seller ON public.property_purchases USING btree (seller_id, created_at DESC);


--
-- Name: idx_property_purchases_status; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_property_purchases_status ON public.property_purchases USING btree (status, created_at DESC);
...
--
-- Name: idx_purchase_events_org_purchase_time; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_purchase_events_org_purchase_time ON public.purchase_events USING btree (organization_id, purchase_id, created_at DESC);
...
CREATE INDEX idx_purchase_events_purchase ON public.property_purchase_events USING btree (purchase_id, created_at DESC);


--
-- Name: idx_purchase_events_purchase_time; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_purchase_events_purchase_time ON public.purchase_events USING btree (purchase_id, created_at DESC);


--
-- Name: idx_purchase_payments_payment; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_purchase_payments_payment ON public.property_purchase_payments USING btree (payment_id);


--
-- Name: idx_purchase_payments_purchase; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_purchase_payments_purchase ON public.property_purchase_payments USING btree (purchase_id);
...
CREATE INDEX idx_test ON public.tenancies USING btree (id);
...
--
-- Name: uniq_escrow_per_purchase; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uniq_escrow_per_purchase ON public.escrow_accounts USING btree (purchase_id);


--
-- Name: uniq_open_purchase_per_property; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uniq_open_purchase_per_property ON public.property_purchases USING btree (organization_id, property_id) WHERE ((deleted_at IS NULL) AND (status <> ALL (ARRAY['closed'::public.purchase_status, 'cancelled'::public.purchase_status, 'refunded'::public.purchase_status])));


--
-- Name: uniq_open_tenancy; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uniq_open_tenancy ON public.tenancies USING btree (tenant_id, property_id) WHERE ((deleted_at IS NULL) AND (status = ANY (ARRAY['active'::public.tenancy_status, 'pending_start'::public.tenancy_status])));
...
--
-- Name: uniq_rent_invoice_payments_invoice; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: uniq_rent_payments_invoice; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: uniq_wallet_escrow; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uniq_wallet_escrow ON public.wallet_accounts USING btree (organization_id, currency) WHERE (purpose = 'escrow'::public.wallet_account_purpose);


--
-- Name: uniq_wallet_platform; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uniq_wallet_platform ON public.wallet_accounts USING btree (organization_id, currency) WHERE (purpose = 'platform'::public.wallet_account_purpose);


--
-- Name: uniq_wallet_platform_per_org_currency; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uniq_wallet_platform_per_org_currency ON public.wallet_accounts USING btree (organization_id, currency) WHERE ((user_id IS NULL) AND (purpose = 'platform'::public.wallet_account_purpose));
...
--
-- Name: uniq_wallet_tx_purchase_credit; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uniq_wallet_tx_purchase_credit ON public.wallet_transactions USING btree (wallet_account_id, reference_type, reference_id, txn_type) WHERE (reference_type = 'purchase'::text);


--
-- Name: uniq_wallet_tx_rent_invoice_credit; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uniq_wallet_tx_rent_invoice_credit ON public.wallet_transactions USING btree (wallet_account_id, reference_type, reference_id, txn_type) WHERE (reference_type = 'rent_invoice'::text);


--
-- Name: uniq_wallet_user; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX uniq_wallet_user ON public.wallet_accounts USING btree (organization_id, user_id, currency) WHERE (purpose = 'user'::public.wallet_account_purpose);
...
--
-- Name: property_purchases property_purchases_transition_and_log; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER property_purchases_transition_and_log BEFORE UPDATE OF status, payment_status, refund_method, refunded_reason, refunded_by, refund_payment_id ON public.property_purchases FOR EACH ROW EXECUTE FUNCTION public.trg_property_purchases_transition_and_log();
...
--
-- Name: escrow_accounts set_updated_at_escrow_accounts; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER set_updated_at_escrow_accounts BEFORE UPDATE ON public.escrow_accounts FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
--
-- Name: property_purchases set_updated_at_property_purchases; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER set_updated_at_property_purchases BEFORE UPDATE ON public.property_purchases FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
--
-- Name: escrow_accounts escrow_accounts_organization_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.escrow_accounts
    ADD CONSTRAINT escrow_accounts_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organizations(id);


--
-- Name: escrow_accounts escrow_accounts_purchase_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.escrow_accounts
    ADD CONSTRAINT escrow_accounts_purchase_id_fkey FOREIGN KEY (purchase_id) REFERENCES public.property_purchases(id) ON DELETE CASCADE;
...
--
-- Name: escrow_transactions escrow_transactions_organization_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.escrow_transactions
    ADD CONSTRAINT escrow_transactions_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organizations(id);


--
-- Name: escrow_transactions escrow_transactions_purchase_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.escrow_transactions
    ADD CONSTRAINT escrow_transactions_purchase_id_fkey FOREIGN KEY (purchase_id) REFERENCES public.property_purchases(id) ON DELETE CASCADE;


--
-- Name: escrow_wallet_accounts escrow_wallet_accounts_organization_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.escrow_wallet_accounts
    ADD CONSTRAINT escrow_wallet_accounts_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organizations(id);
...
--
-- Name: property_purchase_events property_purchase_events_actor_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchase_events
    ADD CONSTRAINT property_purchase_events_actor_user_id_fkey FOREIGN KEY (actor_user_id) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: property_purchase_events property_purchase_events_organization_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchase_events
    ADD CONSTRAINT property_purchase_events_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organizations(id);


--
-- Name: property_purchase_events property_purchase_events_purchase_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchase_events
    ADD CONSTRAINT property_purchase_events_purchase_id_fkey FOREIGN KEY (purchase_id) REFERENCES public.property_purchases(id) ON DELETE CASCADE;


--
-- Name: property_purchase_payments property_purchase_payments_organization_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchase_payments
    ADD CONSTRAINT property_purchase_payments_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organizations(id);


--
-- Name: property_purchase_payments property_purchase_payments_payment_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchase_payments
    ADD CONSTRAINT property_purchase_payments_payment_id_fkey FOREIGN KEY (payment_id) REFERENCES public.payments(id) ON DELETE RESTRICT;


--
-- Name: property_purchase_payments property_purchase_payments_purchase_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchase_payments
    ADD CONSTRAINT property_purchase_payments_purchase_id_fkey FOREIGN KEY (purchase_id) REFERENCES public.property_purchases(id) ON DELETE CASCADE;


--
-- Name: property_purchases property_purchases_buyer_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchases
    ADD CONSTRAINT property_purchases_buyer_id_fkey FOREIGN KEY (buyer_id) REFERENCES public.users(id) ON DELETE RESTRICT;


--
-- Name: property_purchases property_purchases_listing_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchases
    ADD CONSTRAINT property_purchases_listing_id_fkey FOREIGN KEY (listing_id) REFERENCES public.property_listings(id) ON DELETE SET NULL;


--
-- Name: property_purchases property_purchases_organization_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchases
    ADD CONSTRAINT property_purchases_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organizations(id);


--
-- Name: property_purchases property_purchases_property_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchases
    ADD CONSTRAINT property_purchases_property_id_fkey FOREIGN KEY (property_id) REFERENCES public.properties(id) ON DELETE RESTRICT;


--
-- Name: property_purchases property_purchases_refund_payment_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchases
    ADD CONSTRAINT property_purchases_refund_payment_id_fkey FOREIGN KEY (refund_payment_id) REFERENCES public.payments(id) ON DELETE SET NULL;


--
-- Name: property_purchases property_purchases_refunded_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchases
    ADD CONSTRAINT property_purchases_refunded_by_fkey FOREIGN KEY (refunded_by) REFERENCES public.users(id) ON DELETE SET NULL;


--
-- Name: property_purchases property_purchases_seller_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.property_purchases
    ADD CONSTRAINT property_purchases_seller_id_fkey FOREIGN KEY (seller_id) REFERENCES public.users(id) ON DELETE RESTRICT;
...
--
-- Name: purchase_events purchase_events_actor_user_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.purchase_events
    ADD CONSTRAINT purchase_events_actor_user_id_fkey FOREIGN KEY (actor_user_id) REFERENCES public.users(id);


--
-- Name: purchase_events purchase_events_organization_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.purchase_events
    ADD CONSTRAINT purchase_events_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organizations(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.purchase_events
    ADD CONSTRAINT purchase_events_purchase_id_fkey FOREIGN KEY (purchase_id) REFERENCES public.property_purchases(id) ON DELETE CASCADE;


--
-- Name: rent_invoice_payments rent_invoice_payments_invoice_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: rent_invoice_payments rent_invoice_payments_organization_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: rent_invoice_payments rent_invoice_payments_rent_payment_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: rent_payments rent_payments_invoice_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: rent_payments rent_payments_organization_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: escrow_accounts escrow_accounts_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY escrow_accounts_select ON public.escrow_accounts FOR SELECT USING (((organization_id = public.current_organization_uuid()) AND (EXISTS ( SELECT 1
   FROM public.property_purchases pp
  WHERE ((pp.id = escrow_accounts.purchase_id) AND (pp.deleted_at IS NULL) AND (public.is_admin() OR (pp.buyer_id = public.current_user_uuid()) OR (pp.seller_id = public.current_user_uuid()) OR public.is_property_owner_or_default_agent(pp.property_id)))))));


--
-- Name: escrow_accounts escrow_accounts_write; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY escrow_accounts_write ON public.escrow_accounts FOR INSERT WITH CHECK ((public.is_admin() OR (CURRENT_USER = 'rentease_service'::name)));
...
--
-- Name: escrow_transactions escrow_transactions_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY escrow_transactions_select ON public.escrow_transactions FOR SELECT USING (((organization_id = public.current_organization_uuid()) AND (EXISTS ( SELECT 1
   FROM public.property_purchases pp
  WHERE ((pp.id = escrow_transactions.purchase_id) AND (pp.deleted_at IS NULL) AND (public.is_admin() OR (pp.buyer_id = public.current_user_uuid()) OR (pp.seller_id = public.current_user_uuid()) OR public.is_property_owner_or_default_agent(pp.property_id)))))));


--
-- Name: escrow_transactions escrow_transactions_write; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY escrow_transactions_write ON public.escrow_transactions FOR INSERT WITH CHECK ((public.is_admin() OR (CURRENT_USER = 'rentease_service'::name)));
...
--
-- Name: property_purchase_events; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.property_purchase_events ENABLE ROW LEVEL SECURITY;

--
-- Name: property_purchase_payments; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.property_purchase_payments ENABLE ROW LEVEL SECURITY;

--
-- Name: property_purchases; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.property_purchases ENABLE ROW LEVEL SECURITY;

--
-- Name: property_purchases property_purchases_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY property_purchases_select ON public.property_purchases FOR SELECT USING (((deleted_at IS NULL) AND (organization_id = public.current_organization_uuid()) AND (public.is_admin() OR (buyer_id = public.current_user_uuid()) OR (seller_id = public.current_user_uuid()) OR public.is_property_owner_or_default_agent(property_id))));


--
-- Name: property_purchases property_purchases_update; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY property_purchases_update ON public.property_purchases FOR UPDATE USING ((public.is_admin() OR (buyer_id = public.current_user_uuid()) OR (seller_id = public.current_user_uuid()))) WITH CHECK ((public.is_admin() OR (buyer_id = public.current_user_uuid()) OR (seller_id = public.current_user_uuid())));


--
-- Name: property_purchases property_purchases_write; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY property_purchases_write ON public.property_purchases FOR INSERT WITH CHECK ((public.is_admin() OR (buyer_id = public.current_user_uuid())));
...
--
-- Name: purchase_events purchase_events_org_isolation_app; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY purchase_events_org_isolation_app ON public.purchase_events USING ((organization_id = public.current_app_org_uuid()));


--
-- Name: property_purchase_events purchase_events_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY purchase_events_select ON public.property_purchase_events FOR SELECT USING (((organization_id = public.current_organization_uuid()) AND (EXISTS ( SELECT 1
   FROM public.property_purchases pp
  WHERE ((pp.id = property_purchase_events.purchase_id) AND (pp.deleted_at IS NULL) AND (public.is_admin() OR (pp.buyer_id = public.current_user_uuid()) OR (pp.seller_id = public.current_user_uuid()) OR public.is_property_owner_or_default_agent(pp.property_id)))))));


--
-- Name: property_purchase_events purchase_events_write; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY purchase_events_write ON public.property_purchase_events FOR INSERT WITH CHECK ((public.is_admin() OR (CURRENT_USER = 'rentease_service'::name)));


--
-- Name: property_purchase_payments purchase_payments_select; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY purchase_payments_select ON public.property_purchase_payments FOR SELECT USING (((organization_id = public.current_organization_uuid()) AND (EXISTS ( SELECT 1
   FROM public.property_purchases pp
  WHERE ((pp.id = property_purchase_payments.purchase_id) AND (pp.deleted_at IS NULL) AND (public.is_admin() OR (pp.buyer_id = public.current_user_uuid()) OR (pp.seller_id = public.current_user_uuid()) OR public.is_property_owner_or_default_agent(pp.property_id)))))));


--
-- Name: property_purchase_payments purchase_payments_write; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY purchase_payments_write ON public.property_purchase_payments FOR INSERT WITH CHECK ((public.is_admin() OR (CURRENT_USER = 'rentease_service'::name)));


--
-- Name: property_purchases purchases_org_isolation_app; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY purchases_org_isolation_app ON public.property_purchases USING ((organization_id = public.current_app_org_uuid()));


--
-- Name: rent_invoice_payments; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.rent_invoice_payments ENABLE ROW LEVEL SECURITY;

--
-- Name: rent_invoice_payments rent_invoice_payments_select; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: rent_invoice_payments rent_invoice_payments_write_admin_or_service; Type: POLICY; Schema: public; Owner: -
--
...
--
-- Name: rent_payments rent_payments_write_admin_or_service; Type: POLICY; Schema: public; Owner: -
--
...
\unrestrict MAxFJe1S0Gbj2hcQHjUNG2vF4MKbekk5ck8a4jgGRhNca0CuMVfUydmcfDPGfEK
