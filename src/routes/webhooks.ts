 ...
 */
async function resolveTransferWebhookContextV2(pg: any, event: any): Promise<WebhookCtx | null> {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (eventName === "charge.success") {
          ...
          ctx = await resolveTransferWebhookContextV2(pg, event);
        } else {
        ...
        if (eventName === "charge.success") {
          ...
          do {
          const res = await finalizePaystackTransferEvent(pg, { organizationId: ctx.organizationId,
