...
async function upsertRefundMilestones(
  ...
  await client.query(
    ...
    [
      args.amount,
      args.refundMethod ?? null,     // ✅ refund_method = refundMethod (card/bank_transfer/wallet)
      args.reason ?? null,           // ✅ refunded_reason = reason text
      args.refundedBy ?? null,       // refunded_by uuid
      args.refundPaymentId ?? null,  // refund_payment_id uuid
      args.purchaseId,
      args.organizationId,
    ]
  );
...
export async function refundPropertyPurchase(
  ...
  // ✅ If already refunded: idempotent, but still fill audit fields if empty.
  if (purchase.payment_status === "refunded" || purchase.refunded_at) {
    await upsertRefundMilestones(client, {
      organizationId: args.organizationId,
      purchaseId: purchase.id,
      amount: 0, // don't add more refunded_amount on re-calls
  ...
  // If already has both debit txns, be idempotent (and store audit fields).
  ...
  // Mark refunded + store audit fields
  await upsertRefundMilestones(client, {
    organizationId: args.organizationId,
    purchaseId: purchase.id,
    amount,
...
echo "NEXT:"
echo "  1) npm run dev"
echo "  2) Re-call refund (idempotent) with audit fields:"
