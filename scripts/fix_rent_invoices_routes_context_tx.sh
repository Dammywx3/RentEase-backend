...
BAKDIR="$ROOT/scripts/.bak"
mkdir -p "$BAKDIR"
ts="$(date +%Y%m%d_%H%M%S)"

if [[ -f "$FILE" ]]; then
  cp "$FILE" "$BAKDIR/rent_invoices.ts.bak.$ts"
...
async function ensureAuth(app: FastifyInstance, req: any, reply: any) {
  try {
    ...
    return reply
      .code(401)
      .send({ ok: false, error: "UNAUTHORIZED", message: "Unauthorized" });
  }
}
...
/**
 * IMPORTANT:
 * We use set_config(..., true) which is LOCAL-to-transaction.
 * So you MUST call these AFTER BEGIN, inside the same transaction.
 */
async function applyOrgUserContext(client: PoolClient, req: any) {
  ...
  await client.query(
    "SELECT set_config('request.header.x_organization_id', $1, true);",
    [org]
  );
  await client.query("SELECT set_config('request.jwt.claim.org', $1, true);", [
    org,
  ]);
  await client.query("SELECT set_config('request.jwt.claims.org', $1, true);", [
    org,
  ]);
  await client.query(
  ...
  await client.query("SELECT set_config('rentease.organization_id', $1, true);", [
    org,
  ]);
...
async function applyRoleContext(client: PoolClient, role: string) {
...
export async function rentInvoicesRoutes(app: FastifyInstance) {
  ...
  app.post("/v1/tenancies/:id/rent-invoices/generate", async (req, reply) => {
    ...
    try {
      ...
      try {
        ...
        await applyOrgUserContext(client, req);
        await applyRoleContext(client, req?.user?.role || "");
  ...
  app.post("/v1/rent-invoices/:id/pay", async (req, reply) => {
    ...
    try {
      await client.query("BEGIN");
      await applyOrgUserContext(client, req);
      // role not set yet; we'll set it after DB check

      // DB-role check (avoid TS generic underline)
      const roleRes = await client.query(
        "select role from public.users where id = $1::uuid limit 1;",
        [userId]
      );
      const dbRole = (roleRes.rows[0] as any)?.role as string | undefined;
      ...
      await applyRoleContext(client, "admin");
