...
# ---- 2) Auto-detect payee source table/column
# Prefer properties; fallback to property_listings.
PAYEE_SRC=""
PAYEE_COL=""
...
if [ -z "$PAYEE_COL" ]; then
  echo "❌ Could not auto-detect a payee user id column on public.properties or public.property_listings."
  echo "   Run ONE of these and paste the result:"
...
# ---- 3) Patch rent_invoices.service.ts: resolver uses rent_invoices.property_id -> PAYEE_SRC
python3 - <<PY
import re
...
# Ensure PoolClient import exists (some versions use PoolClient already)
# We'll not force; just use "any" if missing. But your file already imports PoolClient.
...
# remove any platform wallet select + old insert, then insert correct payee wallet tx.
txt = re.sub(
...
echo "➡️ Then generate & pay a NEW invoice to see user_id populated on wallet_accounts."
