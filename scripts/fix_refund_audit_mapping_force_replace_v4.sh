...
# ✅ IMPORTANT: single quotes so $2/$3/$4/$5 are NOT expanded by bash
grep -Eq 'refund_method[[:space:]]*=[[:space:]]*COALESCE\(refund_method,[[:space:]]*\$2::text\)' "$FILE" || {
  ...
  awk 'BEGIN{p=0} /async function upsertRefundMilestones/{p=1} p{print} /async function insertDebitTxn/{exit}' "$FILE" | sed -n '1,220p'
  exit 1
}
...
grep -Eq 'refunded_by[[:space:]]*=[[:space:]]*COALESCE\(refunded_by,[[:space:]]*\$4::uuid\)' "$FILE" || {
  echo "❌ Still missing refunded_by mapping after patch."
  exit 1
}

grep -Eq 'refund_payment_id[[:space:]]*=[[:space:]]*COALESCE\(refund_payment_id,[[:space:]]*\$5::uuid\)' "$FILE" || {
  echo "❌ Still missing refund_payment_id mapping after patch."
  exit 1
}
