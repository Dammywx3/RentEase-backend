...
if [ -z "${BASH_VERSION:-}" ]; then
  echo "❌ Run with bash: DB_NAME=rentease bash scripts/dev_seed_unpaid_purchase_safe_v2.sh"
  exit 1
fi
...
SQL_FILE="scripts/.tmp_seed_unpaid_purchase_safe_v2.${TS}.sql"
...
echo "==> Applying..."
PURCHASE_ID="$(PAGER=cat psql -d "$DB_NAME" -X -v ON_ERROR_STOP=1 -t -A -f "$SQL_FILE")"
PURCHASE_ID="$(echo "$PURCHASE_ID" | tr -d '[:space:]')"

if [ -z "$PURCHASE_ID" ]; then
  echo "❌ Could not create or reuse a purchase (no id returned)."
  echo "Tip: check you have properties+listings and/or any existing purchases."
  exit 1
fi
