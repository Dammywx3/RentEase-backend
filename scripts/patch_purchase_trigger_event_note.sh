...
SQL_FILE="scripts/.tmp_patch_purchase_trigger_event_note.${TS}.sql"
...
echo "✅ Patched trigger: purchase_events.metadata now includes app.event_note when set."
echo "==> Tip: view function with:"
echo "PAGER=cat psql -d \"$DB_NAME\" -X -c \"select pg_get_functiondef('public.trg_property_purchases_transition_and_log()'::regprocedure);\""
