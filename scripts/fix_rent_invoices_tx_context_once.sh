...
cp "$FILE" "$BAK_DIR/rent_invoices.ts.bak.$ts"
...
/**
 * IMPORTANT:
 * We use set_config(..., true) which is LOCAL to the current transaction.
 * Therefore we MUST call BEGIN first, then applyDbContext(), then run queries.
 */
async function applyDbContext(client: PoolClient, req: any) {
  const headers = req?.headers ?? {};
  const orgId =
    headers["x-organization-id"] ??
    headers["x_organization_id"] ??
    headers["X-Organization-Id"];
  ...
  // Must match public.current_organization_uuid()
...
export async function rentInvoicesRoutes(app: FastifyInstance) {
  ...
  app.post("/v1/tenancies/:id/rent-invoices/generate", async (req, reply) => {
    ...
    try {
      ...
      try {
        ...
        return reply.send({ ok: true, reused: !!(res as any)?.reused, data: (res as any)?.row ?? res });
      } catch (err: any) {
  ...
  // -----------------------------
  // List invoices (wrap in tx so LOCAL set_config works)
  // -----------------------------
  ...
  app.post("/v1/rent-invoices/:id/pay", async (req, reply) => {
    ...
    try {
      ...
      // Avoid TS underline by not using generic here
      const roleRes = await client.query(
        "select role from public.users where id = $1::uuid limit 1;",
        [userId]
      );
      const dbRole = (roleRes.rows[0] as any)?.role;
      ...
      const payRes: any = await payRentInvoice(client, {
        invoiceId: params.id,
        paymentMethod: body.paymentMethod,
        amount: body.amount,
      });

      const alreadyPaid = !!payRes?.alreadyPaid || !!payRes?.reused;
      const row = payRes?.row ?? payRes;

      await client.query("COMMIT");
      return reply.send({ ok: true, alreadyPaid, data: row });
    } catch (err: any) {
      try { await client.query("ROLLBACK"); } catch {}
      return reply.code(500).send({
        ok: false,
...
echo "✅ Fixed transaction + db context order in: src/routes/rent_invoices.ts"
