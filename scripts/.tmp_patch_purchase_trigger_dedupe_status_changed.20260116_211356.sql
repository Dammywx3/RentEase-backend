...
-- Patch: dedupe status_changed events when status was auto-synced from payment_status
-- Assumes you already have:
--  - public.current_app_user_uuid()
--  - public.assert_purchase_transition(old_status, new_status, old_pay, new_pay)
--  - purchase_events.event_type is purchase_audit_event_type (or compatible)
--  - property_purchases trigger calls trg_property_purchases_transition_and_log()
...
-- Verify trigger exists and points to this function
SELECT
  tgname,
  tgenabled,
